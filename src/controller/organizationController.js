...
const createOrganization = async (req, res) => {
  try {
    ...
    if (!organizationName) {
      return res
        .status(400)
        .send({ status: false, message: "organizationName is mandatory" });
    }
    if (!isValidName(organizationName)) {
      return res
        .status(400)
        .send({ status: false, message: "organizationName is invalid" });
    }
    let organizationNames = await organizationModel.findOne({
      organizationName,
    });
...
const organizationLogin = async (req, res) => {
    try {
        ...
        return res.status(200).send({status: true,message: "Organization login successfully",data: { token: token },
        })
